...
contract('BNOXToken', function(accounts) {
	it("243. test of BNOXToken: Fee calculation 800 - 0.040% - 0.060%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 800,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49999200, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 799, "BNOX token allocation after transfer - to balance");
