...
contract('BNOXToken', function(accounts) {
	it("251. test of BNOXToken: Fee calculation 300 - 0.066% - 0.100%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 300,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49999700, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 300, "BNOX token allocation after transfer - to balance");
