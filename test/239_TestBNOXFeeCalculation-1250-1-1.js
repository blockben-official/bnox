...
contract('BNOXToken', function(accounts) {
	it("239. test of BNOXToken: Fee calculation 125000- 0.001% - 0.001%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 125000,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49875000, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 124997, "BNOX token allocation after transfer - to balance");
			assert.equal(bsoBalance, 1, "BNOX token allocation after transfer - BSO Fee balance");
			assert.equal(feeBalance, 2, "BNOX token allocation after transfer - Processing Fee balance");
