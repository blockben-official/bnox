...
contract('BNOXToken', function(accounts) {
	it("254. test of BNOXToken: Fee calculation 900 - 0.066% - 0.100%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 900,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49999100, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 899, "BNOX token allocation after transfer - to balance");
