...
contract('BNOXToken', function(accounts) {
	it("259. test of BNOXToken: Fee calculation 2100 - 0.066% - 0.100%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 2100,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49997900, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 2097, "BNOX token allocation after transfer - to balance");
