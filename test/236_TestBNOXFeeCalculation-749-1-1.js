...
contract('BNOXToken', function(accounts) {
	it("236. test of BNOXToken: Fee calculation 74900 - 0.001% - 0.001%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 74900,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49925100, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 74899, "BNOX token allocation after transfer - to balance");
