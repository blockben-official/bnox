...
contract('BNOXToken', function(accounts) {
	it("257. test of BNOXToken: Fee calculation 1500 - 0.066% - 0.100%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 1500,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49998500, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 1498, "BNOX token allocation after transfer - to balance");
			assert.equal(bsoBalance, 2, "BNOX token allocation after transfer - BSO Fee balance");
