...
contract('BNOXToken', function(accounts) {
	it("238. test of BNOXToken: Fee calculation 124900- 0.001% - 0.001%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 124900,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49875100, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 124898, "BNOX token allocation after transfer - to balance");
