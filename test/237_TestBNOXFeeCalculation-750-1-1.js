...
contract('BNOXToken', function(accounts) {
	it("237. test of BNOXToken: Fee calculation 75000 - 0.001% - 0.001%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 75000,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49925000, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 74998, "BNOX token allocation after transfer - to balance");
