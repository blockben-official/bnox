...
contract('BNOXToken', function(accounts) {
	it("258. test of BNOXToken: Fee calculation 1600 - 0.066% - 0.100%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 1600,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49998400, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 1597, "BNOX token allocation after transfer - to balance");
