...
contract('BNOXToken', function(accounts) {
	it("231. test of BNOXToken: Fee calculation 10000 - 0.001% - 0.001%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 10000,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49990000, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 10000, "BNOX token allocation after transfer - to balance");
