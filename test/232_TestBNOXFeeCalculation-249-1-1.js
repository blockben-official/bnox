...
contract('BNOXToken', function(accounts) {
	it("232. test of BNOXToken: Fee calculation 24900 - 0.001% - 0.001%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 24900,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49975100, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 24900, "BNOX token allocation after transfer - to balance");
