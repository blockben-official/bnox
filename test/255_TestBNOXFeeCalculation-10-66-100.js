...
contract('BNOXToken', function(accounts) {
	it("255. test of BNOXToken: Fee calculation 1000 - 0.066% - 0.100%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 1000,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49999000, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 998, "BNOX token allocation after transfer - to balance");
