...
contract('BNOXToken', function(accounts) {
	it("260. test of BNOXToken: Fee calculation 2200 - 0.066% - 0.100%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 2200,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49997800, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 2196, "BNOX token allocation after transfer - to balance");
