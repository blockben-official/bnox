...
contract('BNOXToken', function(accounts) {
	it("253. test of BNOXToken: Fee calculation 500 - 0.066% - 0.100%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 500,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49999500, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 499, "BNOX token allocation after transfer - to balance");
