...
contract('BNOXToken', function(accounts) {
	it("234. test of BNOXToken: Fee calculation 49900 - 0.001% - 0.001%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 49900,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49950100, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 49899, "BNOX token allocation after transfer - to balance");
