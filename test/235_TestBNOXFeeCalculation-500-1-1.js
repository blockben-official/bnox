...
contract('BNOXToken', function(accounts) {
	it("235. test of BNOXToken: Fee calculation 50000 - 0.001% - 0.001%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 50000,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49950000, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 49999, "BNOX token allocation after transfer - to balance");
			assert.equal(bsoBalance, 1, "BNOX token allocation after transfer - BSO Fee balance");
			assert.equal(feeBalance, 0, "BNOX token allocation after transfer - Processing Fee balance");
