...
contract('BNOXToken', function(accounts) {
	it("247. test of BNOXToken: Fee calculation 2000 - 0.040% - 0.060%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 2000,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49998000, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 1998, "BNOX token allocation after transfer - to balance");
