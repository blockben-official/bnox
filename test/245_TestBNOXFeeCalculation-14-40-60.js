...
contract('BNOXToken', function(accounts) {
	it("245. test of BNOXToken: Fee calculation 1400 - 0.040% - 0.060%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 1400,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49998600, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 1399, "BNOX token allocation after transfer - to balance");
