...
contract('BNOXToken', function(accounts) {
	it("241. test of BNOXToken: Fee calculation 400 - 0.040% - 0.060%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 400,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 49999600, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 400, "BNOX token allocation after transfer - to balance");
