...
contract('BNOXToken', function(accounts) {
	it("249. test of BNOXToken: Fee calculation 2500000000 - 0.040% - 0.060%", function() {
		return BNOXTokenContract.deployed().then(function(instance) {
			...
			return BNOXTokenContractInstance.mint(accounts[0], 5000000000, {from: accounts[0]});
		}).then(function(result) {
			...
			return BNOXTokenContractInstance.transfer(accounts[1], 2500000000,{from: accounts[0]});
		}).then(function(result) {
			...
			assert.equal(fromBalance, 2500000000, "BNOX token allocation after transfer - from balance");
			assert.equal(toBalance, 2497500000, "BNOX token allocation after transfer - to balance");
			assert.equal(bsoBalance, 1500000, "BNOX token allocation after transfer - BSO Fee balance");
			assert.equal(feeBalance, 1000000, "BNOX token allocation after transfer - Processing Fee balance");
