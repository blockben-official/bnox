...
const ROLE_NAME = 'TREASURY_ADMIN';

async function setTreasuryAdmin() {
  console.log(`Start setting up ${ROLE_NAME}...`);

  const treasuryAdminsEnvList = [
    `${ROLE_NAME}1_ADDRESS`,
    `${ROLE_NAME}2_ADDRESS`,
    `${ROLE_NAME}3_ADDRESS`,
    `${ROLE_NAME}4_ADDRESS`,
  ...
  for (const treasuryAdmin of treasuryAdminsEnvList) {
    console.log(`Grant role ${ROLE_NAME} to address ${process.env[treasuryAdmin]}`);
    const setAdminCall = contract.methods.grantRole(tokenAdminRole, process.env[treasuryAdmin]!);
